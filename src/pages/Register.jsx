  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok || response.status === 201) {
        ...
        setError(data.message || data.error || 'Ro\'yxatdan o\'tish muvaffaqiyatsiz. Iltimos, qayta urinib ko\'ring.')
      }
    } catch (err) {
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[127px]">
          <div className="max-w-md mx-auto">
            <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-lg border border-gray-200">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-black mb-2 text-center">Ro'yxatdan o'tish</h1>
              <p className="text-gray-600 text-sm sm:text-base mb-6 text-center">Yangi hisob yarating</p>
              ...
              <div className="mt-6 text-center">
                <p className="text-sm text-gray-600">
                  Hisobingiz bormi?{' '}
                  <Link to="/login" className="text-barber-olive hover:text-barber-gold font-semibold">
                    Bu yerda kiring
                  </Link>
                </p>
              </div>
