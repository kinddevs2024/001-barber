  ...
  useEffect(() => {
    const fetchBarbers = async () => {
      try {
        ...
        const barbersList = Array.isArray(data) ? data : (data.data || data.barbers || [])
    ...
    <div className="pt-16 sm:pt-20 md:pt-[92px]">
      ...
      <section className="w-full bg-white py-8 sm:py-10 md:py-12 lg:py-16" data-aos="fade-up">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[127px] grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-center">
          <div className="order-2 lg:order-1" data-aos="fade-right">
            <div className="text-xs sm:text-sm font-semibold text-barber-gold mb-3 sm:mb-4 tracking-wider">BIZ HAQIMIZDA</div>
            <h1 className="text-3xl xs:text-4xl sm:text-5xl md:text-5xl lg:text-6xl font-bold text-black mb-4 sm:mb-6 md:mb-8 leading-tight">Litsenziyalangan Professional Barberlar</h1>
            <p className="text-black text-base sm:text-lg mb-4 sm:mb-6 opacity-80">
              Toshkentdagi tajribali barberlar jamoamiz har bir soch olishga yillik tajriba va ehtiros olib keladi. {contactInfo.description} Biz eng yuqori sifatli parvarish xizmatlarini taqdim etishga sodiqmiz.
            </p>
            <Button
              size="lg"
          ...
          <div className="w-full h-[400px] xs:h-[450px] sm:h-[500px] md:h-[600px] lg:h-[700px] rounded-2xl sm:rounded-3xl overflow-hidden order-1 lg:order-2" data-aos="fade-left">
            <img 
              src={imagePool[0]} 
      ...
      <section className="w-full bg-barber-olive py-8 sm:py-10 md:py-12 lg:py-16" data-aos="fade-up">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[127px]">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white text-center mb-6 sm:mb-8 md:mb-12">Bizning Barberlarimiz</h2>
          ...
          {error && !loading && (
            <div className="text-center py-4 mb-6">
              <p className="text-yellow-300 text-sm">⚠️ {error}. Standart ma'lumotlar ishlatilmoqda.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {displayBarbers.map((barber, i) => {
              ...
              const barberRole = barber.role || barber.position || barber.specialty || 'Barber'
              const barberDescription = barber.description || barber.bio || barber.about || 'Professional barber with years of experience.'
              const barberImage = barber.image || barber.photo || barber.avatar || getImagesInOrder(displayBarbers.length)[i]
              const instagramUrl = barber.social?.instagram || barber.instagram || barber.socialMedia?.instagram || 'https://www.instagram.com/001_barbershop_'
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-base sm:text-lg md:text-xl font-bold text-white mb-1 uppercase">{barberName}</h3>
                  <p className="text-barber-gold mb-2 sm:mb-3 md:mb-4 text-sm sm:text-base">{barberRole}</p>
                  ...
                  <div className="flex gap-2 sm:gap-3 md:gap-4">
                    {[...Array(3)].map((_, j) => (
                      ...
                      >
                        <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
      ...
      <section className="w-full bg-barber-dark py-8 sm:py-10 md:py-12 lg:py-16" data-aos="fade-up">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[127px]">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-3 sm:mb-4 md:mb-6 text-left">To'liq xizmat ko'rsatadigan barbershop va erkaklar parvarish studiyasi</h2>
          <p className="text-white text-sm sm:text-base md:text-lg mb-4 sm:mb-6 md:mb-8 text-left opacity-90 max-w-3xl">
            Toshkentdagi {contactInfo.tagline} da biz to'liq parvarish xizmatlari assortimentini taklif qilamiz. Klassik soch olishdan zamonaviy uslublargacha, issiq sochiq bilan qirqishdan soqol tuzatishgacha, bizning malakali barberlarimiz sizga eng yaxshi ko'rinish va his qilishda yordam berish uchun bu yerda.
          </p>
