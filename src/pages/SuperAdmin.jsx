...
function SuperAdmin() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      try {
        ...
        response = await apiRequest('/users/admins', {
          method: 'GET'
        }, false)
        ...
        try {
          ...
          response = await apiRequest('/users?role=admin', {
            method: 'GET'
          }, false)
      ...
      const adminsList = Array.isArray(data) ? data : (data.data || data.admins || data.users || [])
  ...
  const handleAddAdmin = async (e) => {
    ...
    try {
      ...
      if (response.ok || response.status === 201) {
        ...
        setError(data.message || data.error || 'Admin qo\'shish muvaffaqiyatsiz')
      }
    } catch (err) {
  ...
  const handleDeleteAdmin = async (adminId) => {
    ...
    try {
      ...
      const response = await apiRequest(`/users/${adminId}`, {
        method: 'DELETE'
      }, false)
      ...
      <section className="w-full py-8 sm:py-10 md:py-12 lg:py-16">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[127px]">
          <div className="flex justify-between items-center mb-10">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-black">Super Admin Boshqaruv Paneli</h1>
          ...
          {showAddForm && (
            <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-lg border border-gray-200 mb-8">
              <h2 className="text-xl sm:text-2xl font-bold text-black mb-6">Yangi admin qo'shish</h2>
              <form onSubmit={handleAddAdmin} className="space-y-4 sm:space-y-5 md:space-y-6">
                <Input
                  type="text"
                  name="name"
                  value={formData.name}
                  ...
                  {admins.length === 0 ? (
                    ...
                    admins.map((admin) => (
                      <tr key={admin.id || admin._id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm">{admin.id || admin._id}</td>
                        <td className="px-4 py-3 text-sm">{admin.name || admin.fullName || 'N/A'}</td>
                        <td className="px-4 py-3 text-sm">{admin.tg_username || admin.telegram_username || 'N/A'}</td>
                        <td className="px-4 py-3 text-sm">{admin.phone_number || admin.phone || 'N/A'}</td>
